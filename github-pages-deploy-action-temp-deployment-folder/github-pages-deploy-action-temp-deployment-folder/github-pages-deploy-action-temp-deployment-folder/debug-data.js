...
async function debugDataMismatch() {
	...
	console.log(
		"Total countries in countryData:",
	...
	queryResult.slice(0, 10).forEach((country, index) => {
		...
		console.log(
			`${index + 1}. ${country.name} (${
				country.ISO_A3
			}) - Has data: ${hasCountryData}`
		);
	});

	// Count how many query results have corresponding country data
	const matchingResults = queryResult.filter(
		(result) => countryData[result.ISO_A3]
	);
	console.log(
	...
	sampleCountries.forEach((country) => {
		console.log(
			`${country.name} (${country.ISO_A3}): ${
				country.flagDescription ? "Has flag description" : "No flag description"
			}`
		);
	});
	...
	const redFlagCountries = countries.filter(
		(country) =>
	...
	redFlagCountries.forEach((country) => {
		console.log(
			`${country.name} (${country.ISO_A3}): ${country.flagDescription.substring(
				0,
				100
			)}...`
		);
